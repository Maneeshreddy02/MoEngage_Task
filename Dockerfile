    ...
    COPY --from=builder /app/target/moengage-task-1.0-SNAPSHOT.war /app/moengage-task.war # This line copies the WAR

    # Expose the port webapp-runner will listen on
    EXPOSE 8080

    # Define the command to run the application
    CMD ["java", "-jar", "webapp-runner.jar", "--port", "$PORT", "moengage-task.war"] # This line starts the WAR